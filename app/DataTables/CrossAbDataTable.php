...
class CrossAbDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query, Request $request): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('number', function ($abc_detail) {
                ...
                $ab = $abc_detail->drawDetail?->ab ?? null;

                if ($ab !== null && $number == $ab) {
    ...
    public function query(CrossAbcDetail $model, Request $request): QueryBuilder
    {
        ...
        $game_id = $request->get('game_id'); // capture game_id if provided
        ...
        $builder = $model->newQuery()
          ...
          ->when(request()->segment(1) !== 'admin' && $this->resolveUserId(), function ($q) {
...
public function html(): HtmlBuilder
{
    ...
    $adminName = 'admin.dashboard.cross.get.ab';
    $frontName = 'dashboard.draw.cross.ab.list';
    $fallbackPath = url('/dashboard/cross-ab-list');
