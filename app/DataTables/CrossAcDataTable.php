...
class CrossAcDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query, Request $request): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('number', function ($abc_detail) {
                ...
                $ac = $abc_detail->drawDetail?->ac ?? null;

                if ($ac !== null && $number == $ac) {
    ...
    public function query(CrossAbcDetail $model, Request $request): QueryBuilder
    {
        ...
        $builder = $model->newQuery()
            ...
            // keep non-admin user filtering as original (applies only when not admin)
 ...
 public function html(): HtmlBuilder
{
    ...
    $adminName = 'admin.dashboard.cross.get.ac';
    $frontName = 'dashboard.draw.cross.ac.list';
    $fallbackPath = url('/dashboard/cross-ac-list');
