    ...
    @push('custom-js')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                window.__drawsDtApi = (dtWrapper && (dtWrapper.api ? dtWrapper.api() : dtWrapper)) || window
                    .__drawsDtApi || null;
                ...
                (function attachMutationObserverForTotals() {
                    function getDtApiForReload() {
                        ...
                        if (window.LaravelDataTables && window.LaravelDataTables['draw-details-table']) {
                            try {
                                return window.LaravelDataTables['draw-details-table'].api();
                            } catch (e) {}
                        }
                        try {
                            return (window.$ && $('#draw-details-table').DataTable) ? $('#draw-details-table')
                                .DataTable() : null;
                        } catch (e) {}
                        return null;
                    }
                    ...
                    try {
                        const possibleHeaders = Array.from(document.querySelectorAll(
                            'h4, h3, .card-header, .card-title, .panel-heading'));
                        totalsPanel = possibleHeaders.find(h => h.textContent && h.textContent.trim().toLowerCase()
                            .includes('total admin'));
                        if (totalsPanel) totalsPanel = totalsPanel.closest('.card') || totalsPanel.parentElement;
                        else totalsPanel = document.querySelector('.col-4') || document.querySelector(
                            '.right-panel') || document.querySelector('.sidebar-right');
                    } catch (e) {
                        totalsPanel = null;
                    }
                ...
                $('#draw-details-table').off('xhr.dt.tableTotals').on('xhr.dt.tableTotals', function(e, settings,
                json) {
                    if (!json) {
                        serverTotals = null;
                        removeTotalsRow();
                ...
                (function attachDateFilterHandlers() {
                    ...
                    function getDayToken() {
                        return document.querySelector('input[name="day"]')?.value || '';
                    }

                    $('#draw-details-table').off('preXhr.dt.dateFilter').on('preXhr.dt.dateFilter', function(e,
                        settings, data) {
                ...
                function calculateTotalsFromDom() {
                    ...
                    dtApi.rows({
                        ...
                        totals.tq += chooseSmallestUnder(parseNumbersFromString($tds.eq(IDX_TQ).text()),
                            LIMIT_COUNT);

                        // CLAIM: visible numbers only and must be <= LIMIT_COUNT
                        totals.claim_amt += chooseSmallestUnder(parseNumbersFromString($tds.eq(IDX_CLAIM)
                        .text()), LIMIT_COUNT);

                        // CROSS AMT: prefer data-value then visible
                        const dv = $tds.eq(IDX_CROSS).find('[data-value]').attr('data-value') || $tds.eq(
                            IDX_CROSS).attr('data-value') || '';
                        const crossNums = (dv ? parseNumbersFromString(dv) : []).concat(parseNumbersFromString(
                            $tds.eq(IDX_CROSS).text()));
                        ...
                        totals.cross_claim += chooseSmallestUnder(parseNumbersFromString($tds.eq(
                            IDX_CROSS_CLAIM).text()), LIMIT_COUNT);

                        totals.p_and_l += (parseNumbersFromString($tds.eq(IDX_PL).text()).length ?
                            parseNumbersFromString($tds.eq(IDX_PL).text())[0] : 0);
                    });
                ...
                function renderServerTotalsRow(serverTotals, clientTotals) {
                    ...
                    const cross = Number(serverTotals.cross ?? serverTotals.cross_total ?? serverTotals.cross_amt ??
                        clientTotals.cross ?? 0);
    ...
    <tr id="totals-row" class="totals-row">
        ...
        <td style="background:#2c3e50;color:#fff;font-weight:700">--</td>
                ...
                if (dtApi.on) {
                    ...
                    $(dtApi.table().node()).on('draw.dt', function() {
                        addTotalsRow();
                    });
                }
                ...
                $(document).on('click', '.addClaim', function(ev) {
                    ...
                    try {
                        if (typeof Livewire !== 'undefined' && typeof Livewire.emit === 'function') {
                            Livewire.emit('openClaimModal', {
                                draw_details_id: drawDetailId
                            });
                            return;
                        }
                    } catch (err) {}
                    ...
                    const domEvent = new CustomEvent('claim-event', {
                        detail: {
                            draw_details_id: drawDetailId
                        }
                    });
...
(function($){
  ...
  // Keep references to DataTables to destroy safely
  let claimDatatables = [];
  let ticketDatatables = [];
  // Save current claim view to restore on Back
  let savedClaimView = null;

  function destroyDts(arr){
    if(!Array.isArray(arr)) return;
    while(arr.length){
      const dt = arr.pop();
      try { if(dt && dt.destroy) dt.destroy(true); } catch(e){ console.warn('dt destroy err', e); }
    }
  }

  function fullCleanup($modal){
    // destroy tracked datatables
    destroyDts(claimDatatables);
    destroyDts(ticketDatatables);
    // clear modal content
    $modal.find('.modal-body').empty();
    $modal.find('.modal-title').text('');
    $modal.find('.modal-footer').empty();
    // remove any leftover backdrops and classes
    $('.modal-backdrop').remove();
    $('body').removeClass('modal-open').css('padding-right','');
    // hide the modal element to be safe
    $modal.hide();
    savedClaimView = null;
  }

  function showBsModal($modal){
    // always create new instance (bootstrap 5)
    try {
  ...
  function initClaimTableDts($modal){
    // dest existing claim datatables and re-init any table with id #claimDetailsTable
    destroyDts(claimDatatables);
    if($.fn.dataTable && $modal.find('#claimDetailsTable').length){
      try {
        const dt = $modal.find('#claimDetailsTable').DataTable({
          paging: false, searching: true, ordering: true, info: false, autoWidth: false
        });
        claimDatatables.push(dt);
      } catch(e){ console.warn('init claim dt err', e); }
    }
  }

  function initTicketTableDts($modal){
    destroyDts(ticketDatatables);
    if($.fn.dataTable){
      $modal.find('table').each(function(){
        const $t = $(this);
        if(!$t.hasClass('no-datatable') && !$t.hasClass('dataTable')){
          try {
            const dt = $t.DataTable({ paging:false, searching:true, ordering:true, info:false, autoWidth:false });
            ticketDatatables.push(dt);
          } catch(e){ /* not all tables are datatables */ }
        }
      });
    }
  }
  ...
  // On document ready wire events
  $(function(){
    const $modal = $('#claimDetailsModal');
    if(!$modal.length){
      console.warn('claimDetailsModal not found - consolidated modal script expects #claimDetailsModal in DOM.');
      return;
    }

    // Delegated click: open claim list
    $(document).off('click.claimOpen', 'a.claim-open').on('click.claimOpen', 'a.claim-open', function(e){
      e.preventDefault();
      const $el = $(this);
      ...
      // prepare modal
      $modal.find('.modal-title').text((type === 'claim' ? 'Claim' : 'Cross Claim') + ' of draw ' + (drawTime || ('#' + drawDetailId)));
      $modal.find('.modal-body').html('<div class="text-center p-3">Loading claim details…</div>');
      $modal.find('.modal-footer').html('<button type="button" class="btn btn-secondary btn-close-modal">Close</button>');

      // show modal
      showBsModal($modal);

      // fetch claim data (server returns JSON with data[])
      ...
      $.ajax({
        ...
        if(res && res.error){
          $modal.find('.modal-body').html('<div class="alert alert-danger">' + escapeHtml(res.error) + '</div>');
          return;
        }
        if(!res || !Array.isArray(res.data) || !res.data.length){
          $modal.find('.modal-body').html('<div class="text-center">No tickets found</div>');
          return;
        }

        const rows = res.data;
        let html = '<div class="table-responsive"><table id="claimDetailsTable" class="table table-striped table-bordered"><thead><tr>' +
                   '<th>#</th><th>Username</th><th>Ticket No</th><th>TQ</th><th>Claim</th><th>Cross Amt</th><th>Cross Claim</th><th>P & L</th></tr></thead><tbody>';
        rows.forEach(function(r, idx){
          const username = r.username || 'N/A';
          const ticketId = r.ticket_id ?? r.id ?? '';
          const userId = r.user_id ?? '';
          const drawId = r.draw_detail_id ?? '';
          ...
          const ticketLink = `<a href="#" class="open-ticket" data-draw="${escapeHtml(drawId)}" data-ticket="${escapeHtml(ticketId)}" data-user="${escapeHtml(userId)}">${escapeHtml(ticketNo)}</a>`;

          html += `<tr>
            <td>${idx+1}</td>
            <td>${escapeHtml(username)}</td>
            <td>${ticketLink}</td>
            <td>${tq.toLocaleString()}</td>
            <td>${claim.toLocaleString()}</td>
            <td>${crossAmt.toLocaleString()}</td>
            <td>${crossClaim.toLocaleString()}</td>
            <td>${pl.toLocaleString()}</td>
          </tr>`;
        });
        ...
        // (re)initialize claim datatable if available
        initClaimTableDts($modal);

        // ensure Close works
        $modal.find('.btn-close-modal').off('click.close').on('click.close', function(){
          const inst = bootstrap.Modal.getInstance($modal[0]);
          if(inst) inst.hide();
          fullCleanup($modal);
          // optional: refresh main table if available
          try { if(window.__drawsDtApi && window.__drawsDtApi.ajax) window.__drawsDtApi.ajax.reload(null,false); } catch(e){}
        });
        ...
        try {
          ...
          else if(xhr.status === 404) msg = 'Not found (404)';
          else if(xhr.status === 500) msg = 'Server error (500)';
        } catch(e){}
        $modal.find('.modal-body').html('<div class="alert alert-danger">' + escapeHtml(msg) + '</div>');
      });

    });

    // Delegated click: open ticket details (inside the claim modal)
    $(document).off('click.openTicket', '#claimDetailsModal .open-ticket').on('click.openTicket', '#claimDetailsModal .open-ticket', function(e){
      e.preventDefault();
      const $link = $(this);
      const drawId = $link.data('draw');
      const ticketId = $link.data('ticket');
      const userId = $link.data('user');

      if(!ticketId){
        console.warn('open-ticket missing ticket id');
        return;
      }

      // Save claim view to restore on Back
      savedClaimView = {
        title: $modal.find('.modal-title').html(),
        body: $modal.find('.modal-body').html(),
        footer: $modal.find('.modal-footer').html()
      };

      // Show loading state
      $modal.find('.modal-title').text('Ticket #' + (ticketId || ''));
      $modal.find('.modal-body').html('<div class="text-center p-3">Loading ticket details…</div>');
      $modal.find('.modal-footer').html('<button type="button" class="btn btn-secondary btn-ticket-back">Back</button> <button type="button" class="btn btn-secondary btn-close-modal">Close</button>');

      // Fetch ticket HTML (your existing route)
      ...
      fetch(url, { credentials: 'same-origin' })
        ...
        .then(html => {
          $modal.find('.modal-body').html(html);
          // Init any datatables inside ticket view
          initTicketTableDts($modal);

          // Wire Back
          $modal.find('.btn-ticket-back').off('click.back').on('click.back', function(){
            // destroy ticket datatables
            destroyDts(ticketDatatables);
            // restore claim html
            if(savedClaimView){
              $modal.find('.modal-title').html(savedClaimView.title || '');
              $modal.find('.modal-body').html(savedClaimView.body || '');
              $modal.find('.modal-footer').html(savedClaimView.footer || '<button type="button" class="btn btn-secondary btn-close-modal">Close</button>');
              // re-init claim dt
              initClaimTableDts($modal);
              savedClaimView = null;
            } else {
        ...
        .catch(err => {
          ...
          $modal.find('.modal-body').html('<div class="alert alert-danger">Failed to load ticket details</div>');
          $modal.find('.modal-footer').html('<button type="button" class="btn btn-secondary btn-close-modal">Close</button>');
          $modal.find('.btn-close-modal').off('click.close').on('click.close', function(){
            const inst = bootstrap.Modal.getInstance($modal[0]);
            if(inst) inst.hide();
    ...
    // Ensure real cleanup on modal hidden
    $modal.off('hidden.bs.modal.cleanup').on('hidden.bs.modal.cleanup', function(){
      // give Bootstrap a tick to remove backdrop then fully cleanup
      setTimeout(function(){
        fullCleanup($modal);
      }, 50);
    });
    ...
    <!-- Claim Details Modal -->
    <div id="claimDetailsModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="claimDetailsModalLabel"
        aria-hidden="true">
